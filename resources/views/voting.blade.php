...
<body class="bg-primary" data-turbo="false">
    <div class="flex flex-col min-h-screen">
        <main class="flex-grow flex items-center justify-center">
            <div class="mx-auto w-full">
                <div class="text-center px-6 lg:p-0 mb-6 lg:mb-12">
                    <h1 class="text-2xl lg:text-4xl font-bold text-accent mb-2 lg:mb-2">
                ...
                <form id="votingForm" class="space-y-8">
                    <div class="flex flex-nowrap gap-2 lg:gap-8 items-center justify-center" x-data="voting">
                        <template x-for="(calon, index) in calons" :key="calon.id">
                            <div :id="'caketos-container-' + (index + 1)" class="caketos-item">
                                <div class="cursor-pointer flex w-[10rem] lg:w-[22rem] group items-center relative">
                                    <div class="bg-white z-10 card w-full border-2 border-gray-200 rounded-xl shadow-lg hover:shadow-xl hover:border-birupesat cursor-pointer transition-all duration-300 overflow-hidden max-w-sm group relative"
                                        :data-calon-id="calon.id" :data-visi="calon.visi" :data-misi="calon.misi"
                                        :data-nama="calon.nama" :data-kelas="calon.kelas">
                                        <i data-lucide="circle-check"
                                            class="selection-indicator opacity-0 text-birupesat absolute top-2.5 right-2.5 text-lg lg:text-2xl z-20 transition-opacity duration-150 ease-in-out"></i>

                                        <input type="radio" name="id_calon" :value="calon.id"
                                            :id="'calon_' + calon.id" class="hidden candidate-radio" />

                                        <label :for="'calon_' + calon.id" class="cursor-pointer block">
                                            <div class="flex gap-3 p-3 lg:p-6 border-b border-gray-100">
                                                <h3 class="font-bold text-lg lg:text-2xl leading-5 lg:leading-6">
                                                    <span x-text="calon.firstName"></span><br />
                                                    <span class="text-gray-500 text-sm lg:text-xl font-medium"
                                                        x-text="calon.lastName"></span>
                                                </h3>
                                            </div>
                                            <div
                                            ...
                                            <div class="p-3 lg:p-6 space-y-3">
                                                <div class="flex justify-between text-sm lg:text-xl">
                                                    ...
                                                    <span class="text-accent font-semibold" x-text="calon.kelas"></span>
                                                </div>
                                            </div>
                                        </label>
                                    </div>
                                    ...
                                    <div class="detail-panel absolute top-[5%] left-0 w-[10rem] lg:w-[22rem] h-[90%] bg-white border-2 border-birupesat rounded-xl shadow-xl overflow-hidden pointer-events-none z-0"
                                        ...
                                        <div class="detail-panel-scroll p-4 pl-7 lg:p-6 lg:pl-9 h-full overflow-y-auto">
                                            <div class="mb-4">
